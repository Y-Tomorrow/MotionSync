...
**⚠️ 重要提示**:
- 如果服务器监听 `0.0.0.0:8080`，局域网内其他设备可以通过 `http://你的IP:8080` 访问
- **所有计算、训练、文件操作都在运行服务器的电脑上执行**，会占用该电脑的CPU/GPU、内存和磁盘空间
- 建议仅在信任的网络环境中开放访问，或限制为仅本机访问（修改为 `host='127.0.0.1'`）

**多人使用说明**:
- ✅ 多人可以同时访问网站和使用界面
- ⚠️ **并发限制**：同一类型的任务同一时间只能运行一个
  - 位姿提取和格式转换共享资源，不能同时运行
  - 视频标注、实时标注、动作训练各自独立，但同一类型只能运行一个
- 💡 **资源消耗**：多人同时使用会显著增加服务器资源消耗，建议根据服务器性能控制并发用户数量
...
## 改进思路
- 将视频动作复刻到游戏中，用除动作外其他方法（如画面匹配）进行补偿提高定位及动作准确度
- 多任务并发，引入任务队列系统，数据库存储任务状态，实现任务id机制，添加线程池管理
- 文本文件以及模型仓库可视化